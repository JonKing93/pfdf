...
class TestIndices:
    def test_mins(_):
        ...
        sout, cout = _clip._indices(current, clipped, 20)
    ...
    def test_maxs(_):
        ...
        clipped = np.arange(0, 10)
        sout, cout = _clip._indices(current, clipped, 10)
    ...
    def test_both(_):
        ...
        length = 10
        sout, cout = _clip._indices(current, clipped, length)
...
class TestExterior:
    def test_keyword_nodata(_):
        ...
        values = _clip._exterior("", araster, rows, cols, nodata=np.array(0))
    ...
    def test_missing_nodata(_, assert_contains):
        ...
        with pytest.raises(MissingNoDataError) as error:
            _clip._exterior("raster", araster, rows, cols, nodata=None)
        assert_contains(
    ...
    def test_memory(_, assert_contains):
        ...
        with pytest.raises(MemoryError) as error:
            _clip._exterior("", araster, rows, cols, nodata=np.array(0))
...
class TestInterior:
    def test(_):
        ...
        rows = [7, 2]
        cols = [3, 6]
        values = _clip._interior(araster, rows, cols)
...
class TestValues:
    def test_interior(_):
        ...
        values = _clip.values("", araster, bounds, stransform, nodata=np.array(-9))
    ...
    def test_exterior(_):
        ...
        values = _clip.values("", araster, bounds, affine, nodata=np.array(0))
    ...
    def test_inexact(_, clipped):
        ...
        affine = Transform(1, 1, 0, 0).affine
        values = _clip.values("", araster, bounds, affine, nodata=np.array(-9))
        assert np.array_equal(values, araster[3:8, 2:8])
        assert values.base is araster.base
