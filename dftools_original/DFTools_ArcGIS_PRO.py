...
def CalcPixelLength(in_fdir,out_fdir_length,workingdir):
    ...
    outzlength_calc = arcpy.sa.ReclassByASCIIFile(in_fdir,length_remap,"NODATA")
...
def ExtractFeaturesDiffProj(in_extent_feat,in_extract_feat,out_extract_feat):

    [in_extract_feat_path,in_extract_feat_name] = os.path.split(in_extract_feat)
    [in_extent_feat_path,in_extent_feat_name] = os.path.split(in_extent_feat)

    in_extent_feat_desc = arcpy.Describe(in_extent_feat)
    in_extent_feat_spatial_ref = in_extent_feat_desc.SpatialReference

    in_extract_feat_desc = arcpy.Describe(in_extract_feat)
    in_extract_feat_spatial_ref = in_extract_feat_desc.SpatialReference

    in_extract_box_feat_name = in_extract_feat_name+'_box'
    in_extract_box_feat = os.path.join(in_extent_feat_path,in_extract_box_feat_name)

    out_zextract_feat_name = 'z'+in_extract_feat_name
    out_zextract_feat = os.path.join(in_extract_feat_path,out_zextract_feat_name)

    arcpy.Project_management(in_extent_feat,in_extract_box_feat,in_extract_feat_spatial_ref)

    arcpy.Clip_analysis(in_extract_feat,in_extract_box_feat,out_zextract_feat)

    arcpy.Project_management(out_zextract_feat,out_extract_feat,in_extent_feat_spatial_ref)

    arcpy.Delete_management(out_zextract_feat)
